        ...
        {        
            ...
            foreach (Yarn.Dialogue.RunnerResult step in dialogue.Run(startNode))
            {
                ...
                if (step is Yarn.Dialogue.LineResult) {
                    ...
                    lineResult = SubstituteLineVariables(lineResult);
...
//	                }
                    ...
                    optionSetResult = SubstituteOptionSetVariables(optionSetResult);
...
//                    }
        ...
        Yarn.Dialogue.OptionSetResult SubstituteOptionSetVariables(Yarn.Dialogue.OptionSetResult result){
            for(int i=0; i<result.options.options.Count; i++){
                result.options.options[i] = SubstituteVariables(result.options.options[i]);
            }
            return result;
        }

        Yarn.Dialogue.LineResult SubstituteLineVariables(Yarn.Dialogue.LineResult result){
            result.line.text = SubstituteVariables(result.line.text);
            return result;
        }
        ...
        public string SubstituteVariables(string text){
            ...
            while(true){
                ...
                string variableValue = varStorage.GetValue(variableName).AsString;
